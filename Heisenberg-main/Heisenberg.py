...
def greet():
    ...
    conn.commit()
    conn.close()
    SR.speak("\nMyself Anna. How may I help you?")
...
if __name__=="__main__":
        ...
        root.geometry("{}x{}+{}+{}".format(800,400,int(root.winfo_screenwidth()/2 - 800/2),int(root.winfo_screenheight()/2 - 400/2)))
        root.resizable(0,0)
        root.title("Anna")
        root.iconbitmap('Heisenberg.ico')
        root.configure(bg='#2c4557')
        scrollable_text=scrolledtext.ScrolledText(root,state='disabled',height=15,width=87,relief='sunken',bd=5,wrap=tk.WORD,bg='#c2edda',fg='#800000')
